...
**Recommended order:** `01-meshapi-basics.ipynb` (client + one chat call, nothing else)
→ `02-rag-multiagent.ipynb` (RAG + multi-agent) →
`features.ipynb` (everything else, no topic repeated twice). Imports land at first use
instead of one big upfront block.
...
| Pinecone / Jina keys | **Not needed here** | Only used in `experiments/02-rag-multiagent.ipynb`, which builds its own RAG stack. `features.ipynb` uses MeshAPI's own built-in RAG instead, so it doesn't need them. |
...
The same gateway calls that this notebook walks through one-by-one are wired together in
[`native_rag_app/`](../native_rag_app/): managed RAG, moderation on every question, optional STT in
and TTS out. Write-up: [`06_native_rag_app.md`](06_native_rag_app.md).
