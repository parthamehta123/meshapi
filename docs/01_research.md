...
![Features used vs not used](images/features_used_vs_not_used.png)

**We've used 13 out of 32 individually countable features in the notebooks and app** — the core
"talk to an LLM" path (chat, streaming, tool calling, structured outputs, compare, model discovery,
error handling, the Python SDK itself), **embeddings** (originally through Jina, since switched to
MeshAPI's own `/v1/embeddings`), and — in `native_rag_app/` — MeshAPI's **built-in RAG/file-search**,
**moderations**, **TTS**, and **STT**. Two more (automatic retry/fallback and response caching)
benefit us passively without us configuring anything. Images, video, and the rest of the catalog
are covered in `features.ipynb` even when the app does not use them.
...
| Text-to-speech | Text → spoken audio. | `POST /v1/audio/speech` | ✅ — `native_rag_app/` speaks answers back |
| Speech-to-text | Audio → text transcript (supports speaker labeling). | `POST /v1/audio/transcriptions` | ✅ — `native_rag_app/` transcribes mic questions |
...
## 5.1 Shipped after the first pass

- **Voice in/out on the RAG app** — `native_rag_app/` takes a spoken question (`POST /api/ask-voice`
  plus the mic button), transcribes it with MeshAPI STT, runs the same retrieve → answer path, and
  can speak the reply with TTS. See `docs/06_native_rag_app.md`.
...
- **`client.models.list(provider="mistral")` returns 0 results** even though Mistral models exist —
  ...
  prefix or `.brand` instead (see section 3.1's note, and `experiments/01-meshapi-basics.ipynb` /
  `experiments/02-rag-multiagent.ipynb`, for the working pattern).
- **`client.models.free()` and `.paid()` 404 with `Model not found`** — `GET /v1/models/free` is
  treated as looking up a model whose id is `free`. Use `client.models.list(free=True)` (or filter
  the full list on `m.is_free`) instead. Same trap for `.paid()`.
